...
The default port passed in `create` is `80` (`common::DEFAULT_PORT`), the canonical ruroco port
that the shipped systemd `ruroco.socket` unit listens on. Note that a server started without
socket activation and without a configured `address` binds to `34020`
(`common::FALLBACK_BIND_PORT`) instead; clients must then name the port explicitly.
