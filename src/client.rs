...
use crate::common::{error, hash_public_key, info, time_from_ntp, PADDING_SIZE, RSA_PADDING};
        ...
        CommandsClient::Update => {
            ...
            if !status_code.is_success() {
                let response_text = response
                    .text()
                    .map_err(|e| format!("Could not get text from response: {e}"))?;
                error(&format!("Request failed: {} - {}", status_code, response_text));
                return Ok(());
            }
            ...
            if format!("v{}", env!("CARGO_PKG_VERSION")) == api_data.tag_name {
            ...
            match download_urls.as_slice() {
                ...
                [] => {
                    error(&format!("Could not find {binary_name} on https://github.com/beac0n/ruroco/releases/latest"));
                    Ok(())
                }
            }
        }
