...
what was asked, including a deliberate rollback. Downloads the per-arch/OS binary and its
signature (list of (asset prefix, target name, mode, chown user) driven by `binary_targets`,
looping over commander+server or client+client-ui); **verify Ed25519** against the embedded public
key (`keys/ruroco-release-ed25519.pub.pem`, private key is a CI secret) before touching disk; copy
the existing binary to `.old` (rollback snapshot), then atomically replace the target via
`write_atomic_with_mode` (temp file + rename, exec bits set before the swap), so the target always
holds a complete binary (client `0o755`, server binaries `0o500`, chowned to the `ruroco` user).
