...
Config (`config.rs`): `ConfigServer` holds `commands` (name -> shell string), allowed `ips`,
`config_dir`, `socket_user`/`socket_group`, rate limit. Commands are looked up by `blake2b_u64` of
the name. IPs are `normalize_ip`'d on load.
