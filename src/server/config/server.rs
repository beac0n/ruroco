//! Shared server configuration: the `config.toml` schema (`ConfigServer`) and the server CLI
//! (`CliServer`). Both the server and the commander read this file for network/socket settings.
